...
class Create_Category:
    ...
    def __init__(self,master,catogory_name):
        ...
        self.canvas=Canvas(self.baseFrame,width=200,height=300,bg='white',yscrollcommand=self.my_scrollbar.set,borderwidth=0)
        ...
        self.catogory_label=Label(self.baseFrame,text=self.catogory_name.title(),font=('Helvatica',15,'bold italic'),bg='White',fg='blue')
    ...
    def Save(self):
        ...
        for e in self.tasks_dict:#to change in the dictionary and in database
            self.tasks_dict[e]=(self.tasks_dict[e][0],self.tasks_dict[e][0].get())
            self.c.execute("UPDATE {}  SET status={} WHERE  tasks='{}'".format(self.catogory_name,self.tasks_dict[e][0].get(),e))
    ...
    def Retrive_from_database(self):
        ...
        if len(self.details)>0:
            ...
            for e in self.details:
                ...
                self.widgetlist.append(Checkbutton(self.containerFrame,variable=temp_lst[-1], text=e[1], onvalue=1, offvalue=0,bg='white'))
    ...
    def Delete_window(self):
        ...
        self.delete_button = Button(self.del_wind, text="Delete",command=lambda : self.Delete_task(self.del_list.get(ANCHOR)))
